...
void ProtocolStatus::sendStatusString()
{
	...
	players.append_attribute("peak") = std::to_string(g_game.getPlayersRecord()*10).c_str();
...
void ProtocolStatus::sendInfo(uint16_t requestedInfo, const std::string& characterName)
{
	...
	if (requestedInfo & REQUEST_PLAYERS_INFO) {
		output->addByte(0x20);
		output->add<uint32_t>(g_game.getPlayersOnline()*10);
		output->add<uint32_t>(g_config.getNumber(ConfigManager::MAX_PLAYERS));
		output->add<uint32_t>(g_game.getPlayersRecord()*10);
	}
	...
	if (requestedInfo & REQUEST_EXT_PLAYERS_INFO) {
		...
		//aqui enviar um mapa troll
		uint32_t vari = realspoof;
		for (const auto& it : players) {
			vari--;
			if (vari >= 0) {
